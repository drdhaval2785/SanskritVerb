...
/* halantAcca (1.2.10) */
elseif ( arr($text,'/[iIuUfFx][^+]*['.pc('hl').'][+][s]a/') && $san===1)
{
	$itpratyaya=array_merge($itpratyaya,array("k"));
	$it=array_merge($it,array("k"));
...
if ($sanAdi==="san" && arr($text,'/^['.pc('ac').']/'))
{
	...
	if ( arr($text,'/^[^+]*['.pc('ac').'][+]*[sz]a[+]/') && $san===1 && $fo!=="UrRuY" )
	{
...
elseif ($sanAdi==="san" && arr($text,'/^['.pc('hl').']/'))
{
	...
	/* sArvadhAtukArdhadhAtukayoH (7.3.84) */
	if (arr($text,'/[iIuUfFx][+]isa[+]/') && $fo!=="UrRuY" && $kGiti!==1)
	{
...
if ( sub(array("Ipsi+sa+","Irdsi+sa","di+ditsa+","di+Ditsa+","mi+mitsa+"),blank(0),blank(0),0) && $san===1)
{
	$text = one(array("Ipsi+sa+","Irdsi+sa","di+ditsa+","di+Ditsa+","mi+mitsa+"),array("Ipsa+","Irdsa+","ditsa+","Ditsa+","mitsa+"),0);
...
/* adabhyastAt (7.1.4) */
if ($abhyasta===1 && pr2(array("+"),array("anti","antu","an"),blank(0),array("+"),array("ati","atu","us"),blank(0),$text)!==$text && $san!==1)
{
