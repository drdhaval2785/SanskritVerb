...
elseif ($type==='tiGanta') 
{
	...
	if (in_array($lakAra,array("lfw","lfN","luw","ASIrliN","luN","liw","ArDaDAtukalew"))||$san===1) // checking whether ArdhadhAtuka lakAra or not.
	{
		...
		if ($san===1 && (preg_match('/[uU]$/',$verb_without_anubandha)||$fo==="graha!"||$fo==="guhU!") )
		{
			$id_dhAtu="aniw";
			gui2('7.2.12');
	   }
		/* smipUGraJjavazAM sani (7.2.74) */
		elseif ( in_array($fo,array("zmiN","f","pUN","aYjU!","aSU!")) && $san===1)
		{
			$id_dhAtu="sew";
			gui2('7.2.74');
		}
	   ...
	   /* RddhanoH sye (7.2.70) */
		elseif ( (ends(array($verb_without_anubandha),array("f",),1) || $fo==="hana!") && (in_array($lakAra,array("lfw","lfN")) || $san===1 ))
		{
			$id_dhAtu="sew";
			gui2('7.2.70');
		}
		...
		elseif ( in_array($fo,array("gamx!",))  && (in_array($lakAra,array("lfw","lfN")) || $san===1 ) && $verbpada==="p")
		{
			$id_dhAtu="sew";
			gui2('7.2.58');
		}
...
if (in_array($lakAra,$ArdhadhAtuka_lakAra) || in_array($sanAdi,array("yak","yaN","san")))
{
    ...
    {
        $text=one(array("hveY+"),array("hu+"),0);
    ...
    {
        $text=one(array("ada!+"),array("Gasx!+"),0);
...
{
    /* vija iT (1.2.2) */ 
    if ( in_array($fo,array("o!vijI!")) && in_array($lakAra,$ArdhadhAtuka_lakAra) && $id_dhAtu==="sew" && $id_pratyaya==="sew" && $verbset==="tudAdi")
    {
...
if ($lakAra==="luN")
{
	...
	elseif (in_array($fo,array("lipa!","zica!","hveY")) && in_array($so,$tis) )
	{
		$text = one(array("+cli+"),array("+aN+"),0);
	...
	elseif ( ends(array($fo),array("SriY","sru","dru"),2) && in_array($so,$tiG) )
	{
	...
	elseif ( ends(array($fo),array("kamu!"),2) && in_array($so,$tiG) )
	{
...
elseif (in_array($so,$tiG) && $Nizri!==1 && $verbset==="curAdi" && in_array($fo,$curAdi) && $aG!==1 && !arr($text,'/[+]sic[+]/') && $sanAdi!=="yak")
{
    ...
    if (sub(array("+"),$tiG,blank(0),0) && $sarvadhatuka===1 && $sanAdi!=="yak")
    {
...
elseif ( $sarvadhatuka===1 && $verbset==="divAdi"  && in_array($fo,$divAdi) && sub($divAdi,array("+"),blank(0),0) && sub(array("+"),$tiG,blank(0),0) && $sanAdi==="")
{
...
/* kartari zap (3.1.68) */
elseif ( $sarvadhatuka===1 && sub(array("+"),$tiG,blank(0),0) && $sanAdi!=="yak" && $sanAdi!=="yaNluk")
{
    $text=two(array("+"),$tiG,array("+Sap+"),$tiG,0);
    $text=one(array("+Sap+Sap"),array("+Sap+"),0);
	storedata('3.1.68','sa',0);
...
if ( (in_array($sanAdi,array("Ric","RiN","yak","yaN","yaNluk","san")) || arr($text,'/[+]Ri/') ) && ends(array($verb_without_anubandha),array("e","o","E","O"),0) && !(in_array($fo,array("vyeY","veY","hveY")) && $sanAdi==="yak")  )
{
...
if ($sanAdi==="yaN" && $lakAra!=="")
{
	...
	/* yaGi ca (7.4.30) */
	elseif (arr($text,'/^(['.pc('hl').']{2,})f[+]/'))
	{
		$text=change('/^(['.pc('hl').']{2,})f[+]/','$1ar+');
		storedata('7.4.30','sa',0);		
	}
	/* rIG RtaH (7.4.27) */
	elseif (arr($text,'/^['.pc('hl').'][^+]*f[+]/'))
	{
		$text=change('/([^+]*)f[+]/','$1rI+');
		storedata('7.4.22','sa',0);		
	}
	...
	{
		$text=two(array("jyA","grah","vyaD","vyac","vraSc","pracC","Brasj"),array("+"),array("jiA","gfh","viD","vic","vfSc","pfcC","Bfsj"),array("+"),0);
	...
	elseif ( !itcheck(array("i"),1) && arr($text,'/[nMNYRm]['.pc('hl').'][+]/') )
	{
...
/* iko jhal (1.2.9) */
elseif ( arr($text,'/[iIuUfFx][+][s]/') && $san===1 && $id_dhAtu==="aniw")
{
	$itpratyaya=array_merge($itpratyaya,array("N"));
	$it=array_merge($it,array("N"));
...
/* halantAcca (1.2.10) */
elseif ( arr($text,'/[iIuUfFx][^+]*['.pc('hl').'][+][s]/') && $san===1 && $id_dhAtu==="aniw")
{
	$itpratyaya=array_merge($itpratyaya,array("k"));
	$it=array_merge($it,array("k"));
...
elseif ( arr($text,'/^[^+]*['.pc('ac').'][+]san[+]/') && $san===1 && $id_dhAtu==="aniw")
{
	$text = two($ac,array("+san+"),dirgha($ac),array("+san+"),0);
	storedata('6.4.16','sa',0);
}
/* ApjJapyRdhAmIt (7.4.55) */
elseif ( sub(array("Ap","jYap","fD"),array("+san+"),blank(0),0) && $san===1)
{
	$text = two(array("Ap","jYap","fD"),array("+san+"),array("Ip","jYIp","IrD"),array("+san+"),0);
...
if ($sanAdi==="san")
{
	...
	if (arr($text,'/[iIuUfFx]['.pc('hl').'][+]isa[+]/'))
	{
		$text = change('/[iI](['.pc('hl').'][+]isa[+])/','e$1');
		$text = change('/[uU](['.pc('hl').'][+]isa[+])/','o$1');
		$text = change('/[fF](['.pc('hl').'][+]isa[+])/','ar$1');
		$text = change('/[x](['.pc('hl').'][+]isa[+])/','al$1');
		storedata('7.3.86','sa',0);
	}
	$text = one(array("+isa+"),array("isa+"),0);
...
if ($sanAdi==="san")
{
	...
	/* sanyaGoH (6.1.9) */
	elseif (arr($text,'/^['.pc('ac').']['.pc('hl').']*iza[+]/'))
	{
		storedata('6.1.9','sa',0);		
		$abhyAsa=1;
		$abhyasta=1;
...
if (arr($text,'/\+Ri[c]{0,1}\+/')||$ciN===1||$ciN===2)
{
	...
	/* aco JNiti (7.2.115) */
	if ((arr($text,'/['.pc('ac').']\+Ri[Nc]{0,1}\+/')||arr($text,'/['.pc('ac').']\+i$/')) && !in_array($fo,$curAdi_adanta))
	{
...
if (in_array($so,$tiG) && (arr($text,'/['.pc('hl').'][+]/') || (arr($text,'/['.pc('hl').']$/') && $tusma!==1) ||sub(array("+"),$inbetweenpratyaya,array("+"),0) || sub($hl,array("+"),$vikaraNa,0) || $rudhAdibhyaH===1 || pr2($hl,array("+"),array("va","ma"),blank(count($hl)),array("+"),array("va","ma"),$text)!==$text) ) # $ad === 1 removed because of https://github.com/drdhaval2785/SanskritVerb/issues/318
{
	...
	$text=three(array("+"),$inbetweenpratyaya,array("+"),array("+"),$inbetweeenreplace,array("+"),0);
		...
		if ($nomidelision!==1 && arr($text,'/['.pc('hl').'][+]([c]*)[a][+]/') && arr(array($fo),'/['.pc('hl').']$/') )
		{
...
/* Ato GitaH (7.2.81) */
if ( arr($text,'/[+]yaA/') && in_array($so,$tiG) && $sanAdi==="yak") // bad
{
...
elseif ( in_array("k",$itpratyaya) && in_array($fo,array("brUY","Yizvapa!","yaja!","quvapa!","vaha!","veY","vyeY","hveY","vada!","wuo!Svi!","vaca!","vasa!")) && sub(array("vac","svap","yaj","vap","vah","ve","vye","hve","vad","Sv","vas"),array("+"),blank(0),0))
{
...
/* grahijyAvayivyadhivaSTivicativRzcatipRcCatibhRjjatInAM Giti ca (6.1.16) */
elseif ( in_array($fo,array("jyA","graha!","vaya!","vyaDa!","vaSa!","vyaca!","o!vraScU!","praCa!","Brasja!")) && ((sub(array("jyA","grah","vay","vyaD","vaS","vyac","vraSc","pracC","Brasj"),array("+"),$apit_sArvadhAtuka_pratyayas,0) && $sarvadhatuka===1) || in_array("N",$itpratyaya) || in_array("k",$itpratyaya) || in_array("SnA",$vik)) )
{
...
/* jAgro'viciNNalGitsu (7.3.85) */
// Only coded for Ri and Git. Rest pending.
if ( arr($text,'/[j][A][g][f][+]/') && $bhasyADhe!==1 && (!in_array("N",$it)||$lakAra==="ASIrliN"||$sanAdi==="yak") )
{
    $text = two(array('jAgf'),array("+"),array('jAgar'),array("+"),0);
	storedata('7.3.85','sa',0);
	$jAgro=1;
...
if (($syatAsI===1||$sic===1||$sIyuT===1) && (arr($text,'/^[^+]*['.pc('ac').'][+]/')||in_array($fo,array("hana!","graha!","dfSi!r"))||$_GET['sanAdi']==="Ric") && in_array($vAcya,array("bhAva","karma")))
{
	...
	if ($id_dhAtu!=="aniw" && $id_pratyaya!=="aniw")
	{
		$text = change('/[+](s[^+]*)$/','+i$1');
		storedata('7.2.35','sa',0);
	}
...
/* jJAjanorjA (7.3.79) */
if ( in_array($fo,array("janI!","jYA")) && in_array($so,$tiG) && sub(array("jan","jYA"),array("+"),$shitpratyayareplace,0) && $sanAdi!=="yak")
{
...
/* ayaG yi kGiti (7.4.22) */
elseif ( (in_array($sanAdi,array("yak")) || in_array("N",$itpratyaya) || in_array("N",$itpratyaya) ) && sub(array("SI"),array("+y"),blank(0),0) )
{
...
/* aniditAM hala upadhAyAH kGiti (6.4.24) */ 
if ( in_array($fo,$aniditverbs) && (in_array("N",$itpratyaya) || in_array("k",$itpratyaya)) && !in_array($sanAdi,array("Ric"))  && !($so==="mahiN" && $sanAdi==="") && arr($text,'/[NYRnmM]['.pc('hl').'][+]/')  && !in_array("i",$it) && !($lakAra==="viDiliN" && $sanAdi==="") && !($verbset==="ruDAdi" && $sanAdi==="") && $aniditAm!==1 && !in_array("Sap",$vik) && $sanAdi!=="yaNluk") # For application after NeraniTi
{
	if (in_array($fo,$irendiditverbs) && $lakAra==="luN")
	{
		$text = three(array("N","Y","R","n","m","M"),$hl,array("+a+"),array("","","","","","",),$hl,array("+a+"),0);
...
/* bhUsuvostiGi (7.3.88) */
if ( ($sarvadhatuka===1 || $verbset==="adAdi" || $gAtisthA===1 || $lakAra==="luN") && in_array($fo,array("BU","zUN","asa!")) && arr($text,'/[Bs]U\+/') && $sanAdi!=="yaNluk")
{
...
if (arr($text,'/[^+]*A\+/') && !in_array($lakAra,array("viDiliN","ASIrliN")) && sub(array("A"),array("+"),$halAdi_apit_sArvadhAtuka_pratyayas,0) && (in_array("N",$it)||in_array("k",$it)) && $abhyasta===1 && $ghu!==1 && $sarvadhatuka===1 && $sanAdi!=="yaN" && $sanAdi!=="yaNluk")
{
...
/* hali ca (8.2.77) */
elseif ( in_array($so,$tiG) && arr($text,'/^[^+]*[iuf][rv][+]['.pc('hl').']/') )
{
...
/* akRtsArvadhAtukayordIrghaH (7.4.25) */
elseif (  in_array($so,$tiG) && ($sarvadhatuka!==1||$sanAdi==="yak") && $kRt!==1 && (in_array("N",$itpratyaya) || in_array("k",$itpratyaya)) && $lakAra!=="liw" && !arr($text,'/^[y][a][+][y]/') && sub($ac,array("+"),array("y"),0) && !sub($ac,array("+"),array("y+a"),0) )
{
...
elseif ( $lakAra==="liw" && in_array($fo,array("tF","Pala!","Baja!","trapU!z","YiPalA!")) && !in_array($so,array("tip","mip","sip")) && sub(array("ta+tar","pa+Pal","ta+trap"),array("+"),blank(0),0) )
{
    $text=two(array("ta+tar","pa+Pal","ta+trap"),array("+"),array("ter","Pel","trep"),array("+"),0);
	storedata('6.4.122','sa',0);
}
/* tRRphalabhajatrapazca (6.4.122) */
elseif ( $lakAra==="liw" && in_array($fo,array("tF","Pala!","Baja!","trapU!z","YiPalA!")) && in_array($so,array("sip")) && sub(array("ta+tar+is","pa+Pal+is","ta+trap+is"),array("+"),blank(0),0) )
{
    $text=two(array("ta+tar","pa+Pal","ta+trap"),array("+is"),array("ter","Pel","trep"),array("+is"),0);
	storedata('6.4.122','sa',0);
}
...
/* na zasadadavAdiguNAnAm (6.4.126) */
elseif (arr($text,'/va[+]/') && sub(array("va+vaj","va+vaK","va+vaw","va+vaW","va+vaR","va+van","va+val","va+vaz","va+vam"),array("+"),blank(0),0) && $lakAra==="liw" && !ends(array($so),array("tip","mip"),2) )
{
	storedata('6.4.126','sa',0);
}
/* gamahanajanakhanaghasAM lopaH kGiti (6.4.98) */
...
/* ye vibhASA (6.4.43) */
if (arr($text,'/[jsK]an\+y/') && ($sanAdi==="yak"||pr2(array("jan","san","Kan"),array("+"),$halAdi_apit_sArvadhAtuka_pratyayas,array("jaA","saA","KaA"),array("+"),$halAdi_apit_sArvadhAtuka_pratyayas,$text)!==$text) && (in_array("N",$it)||in_array("k",$it)) && in_array($so,$tiG) )
{
    $text=two(array("jan","san","Kan"),array("+y"),array("jaA","saA","KaA"),array("+y"),1);
	storedata('6.4.43','sa',0);
}
/* tanoteryaki (6.4.44) */
if (arr($text,'/tan\+y/') && $sanAdi==="yak" && in_array($so,$tiG) )
{
...
if ($dhatu===1 && $lakAra!=="liw" && (arr($text,'/[rv]['.pc('hl').']/')|| arr($text,'/[v][n][+]/') ) && $karoti!==1  && !($us==="nis" && in_array($so,$tiG)) && sub(array("i","u","f","x"),array("r","v"),$hl,0) )
{
