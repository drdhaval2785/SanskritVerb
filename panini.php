...
/* kRpo ro laH (8.2.18) */
if (arr($text,'/kfp/') && $caG!==1 && $lakAra!=="liw" && $sanAdi!=="yaN")
{
...
if (arr($text,'/kfp/') )
{
    $text=one(array("carI+kfp","kfp"),array("calI+kxp","kxp"),0);
	storedata('8.2.18','sa',0);
}
/* aniditAM hala upadhAyAH kGiti (6.4.24) */ 
if ( in_array($fo,$aniditverbs) && (in_array("N",$itpratyaya) || in_array("k",$itpratyaya)) && !in_array($sanAdi,array("Ric"))  && !($so==="mahiN" && $sanAdi==="") && arr($text,'/[NYRnmM]['.pc('hl').'][+]/')  && !in_array("i",$it) && !($lakAra==="viDiliN" && $sanAdi==="") && !($verbset==="ruDAdi" && $sanAdi==="") && $aniditAm!==1 && !in_array("Sap",$vik)) # For application after NeraniTi
{
	if (in_array($fo,$irendiditverbs) && $lakAra==="luN")
	{
		$text = three(array("N","Y","R","n","m","M"),$hl,array("+a+"),array("","","","","","",),$hl,array("+a+"),0);
...
/* bhUsuvostiGi (7.3.88) */
if ( ($sarvadhatuka===1 || $verbset==="adAdi" || $gAtisthA===1 || $lakAra==="luN") && in_array($fo,array("BU","zUN","asa!")) && arr($text,'/[Bs]U\+/') )
{
...
if (arr($text,'/[^+]*A\+/') && !in_array($lakAra,array("viDiliN","ASIrliN")) && sub(array("A"),array("+"),$halAdi_apit_sArvadhAtuka_pratyayas,0) && (in_array("N",$it)||in_array("k",$it)) && $abhyasta===1 && $ghu!==1 && $sarvadhatuka===1 && $sanAdi!=="yaN")
{
...
elseif (arr($text,$ras) && $hohante!==1 && $_GET['cond2_16_2_1']!=="2" && !sub(array("UruBinn"),blank(0),blank(0),0) && $sanAdi!=="yaN")
{ 
	$inloop=$text;
    foreach ($text as $value)
    {
