...
if ( (sub($juhotyAdi,array("+Sap+"),$tiG,0) && ends(array($fo),$juhotyAdi,4) && $sarvadhatuka===1 && $verbset==="juhotyAdi") || $zluvat===1)
{
...
if ( sub(array("+Ric+Sap+","+RiN+Sap+"),$tiG,blank(0),0) )
{
	...
	// pending to refractor because not able to locate the vArtika number.
...
if ( (sub(array("zWivu!","klamu!",),array("+"),$shitpratyaya,0) || (sub(array("camu!"),array("+"),$shitpratyaya,0) && $us==="A")) && $lakAra!=="" && ends(array($fo),array("zWivu!","klamu!","camu!"),4) )
{
...
if ( in_array($fo,$aniditverbs) && sub(array("+"),array("Syan","Sna","SnA","Snu","Sa"),array("+"),0) )
{
...
// pending to refracor, because the reference to vArtika is missing. It is SK explanation.
...
{
	liT_ajAdi(); // pending to refractor the function.
	$abhyAsa=1;
}
...
/* kGiti ca (1.1.5) */
elseif ($ardhadhatuka===1 && in_array("N",$itpratyaya) && !in_array($sanAdi,array("Ric")) && pr2(array("i","I","u","U","f","F","x","X"),array("+"),$ArdhadhAtuka_tiG_pratyayas,array("i","I","u","U","f","F","x","X"),array("+i"),$ArdhadhAtuka_tiG_pratyayas,$text)!==$text)
{
	storedata('1.1.5','pa',0);
    $kGiti=1;
}
/* kGiti ca (1.1.5) */
elseif ($ardhadhatuka===1 && in_array("N",$itpratyaya) && pr2(array("i","I","u","U","f","F","x","X"),$hlplus,$ArdhadhAtuka_tiG_pratyayas,array("i","I","u","U","f","F","x","X"),$hlplus,$ArdhadhAtuka_tiG_pratyayas,$text)!==$text)
{
	storedata('1.1.5','pa',0);
    $kGiti=1;
}
/* huznuvoH sArvadhAtuke (6.4.87) */
if ($sarvadhatuka===1 && $jherjus!==1 && sub($hl,array("+nu","+u",),array("+"),0) && sub(array("u+"),$ajAdi_apit_sArvadhAtuka_tiG_pratyayas,blank(0),0) )
{
}
elseif ( $sarvadhatuka===1 && $jherjus!==1 && ((arr($text,'/['.pc('ac').'][+][n][u][+]/') &&sub(array("+nu"),array("+"),$ajAdi_apit_sArvadhAtuka_tiG_pratyayas,0) )|| sub(array("juhu+"),$ajAdi_apit_sArvadhAtuka_tiG_pratyayas,blank(0),0)))
{
...
if ( sub($ik,array("+"),array("s","is"),0) && $sic===1 && in_array($so,$tis) )
{
...
/*if (ends(array($fo),array("qukfY","kfY"),4))
{
	...
	// pending to refractor because of non reference to any sUtra.
...
/* sthAdhvoricca (??) */
// pending to refractor, because not able to trace the sUtra or vArtika.
...
/* patch for vI thali */
// See page 83 of SK part 2
if (sub(prat('ic'),array("+"),$tiG1,0) && $lakAra==="liw" && $so==="sip")
{
	$id_dhAtu="vew";
}
...
/* radhijabhoraci (7.1.61) */
if (sub(array("raD","jaB"),array("+"),$ac,0) && ends(array($fo),array("raDa!","jaBI!"),4) )
{
...
/* pugantalaghUpadhasya ca (7.3.86) */
if ( $atolopa!==1 && $ardhadhatuka===1 && sub(array("i","u","f","x"),$hl,array("+sI","+isI"),0) && !arr($text,"/[+][i][y][+][t]$/")  && $didhI!==1 && ($sIyuT===1||$sic===1) && $vijait!==1 && $kGiti!==1)
{
...
// Pending to refractor, because I am unable to locate the paribhASA
//    echo "<p class = sa >By nimittApAye naimittikasyApyapAyaH (paribhASA) :</p>\n";
//    echo "<p class = sa >निमित्तापाये नैमित्तिकस्याप्यपायः (परिभाषा) :</p>\n";
//    display(0);
}
...
elseif ( (in_array($fo,$ajAdi) || $ajAdyataSTAp===1) && $kevala!==1) 
{
	...
	// pending to refractor because these are explanations.
...
elseif ($gender === "f" && $_GET['cond2_21']==="2" && !sub(array("patn"),array("+"),blank(0),0) )
{
	...
	// pending to refractor the gaNasUtra.
...
elseif ($gender === "f" && sub(array("upAdDyAya","upADyAya"),array("+"),blank(0),0) )
{
	...
	storedata('4.1.49','st',8);	
	storedata('4.1.49-4','sa',0);
	// pending to refractor the second vArtika.
...
elseif ($gender === "f" && sub(array("SvaSura"),array("+"),blank(0),0) )
{
	...
	// pending to refractor because unable to locate vArtika.
...
elseif ($gender === "f" && sub(array("sahitoru","sahoru",),array("+"),blank(0),0) )
{
	...
	// pending to refractor because unable to attend.
...
if (sub(array("H"),array("pAS","kalp","kAmy","ka"),blank(0),0) && $inah !== 1 && $nama1 !== 1 && $nama2 !== 1 && $dvi1===0 && $dvi2===0 && $isu1 ===0 && $isu2 ===0 && $tir1===0 && $tir2===0 && $muhu1 ===0  && $atah ===0)
{
	...
	// Pending to refractor pAzakalpaka..
...
// Pending to refractor anAmnavati..
